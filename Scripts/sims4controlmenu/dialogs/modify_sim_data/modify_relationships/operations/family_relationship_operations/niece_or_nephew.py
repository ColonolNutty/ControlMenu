...
class S4CMSetSimAAsNieceOrNephewToSimBOp(S4CMSetSimAAsRelationToSimBOperation):
    ...
    def get_disabled_text(self, new_niece_or_nephew_sim_info: SimInfo, new_aunt_or_uncle_sim_info: SimInfo) -> Union[LocalizedString, None]:
        if not CommonSimGenealogyUtils.has_mother(new_niece_or_nephew_sim_info) and not CommonSimGenealogyUtils.has_father(new_niece_or_nephew_sim_info):
            return CommonLocalizationUtils.create_localized_string(S4CMSimControlMenuStringId.SIM_NEEDS_TO_HAVE_MOTHER_OR_FATHER_BEFORE_YOU_CAN_ADD_UNCLES_OR_AUNTS_TO_THEM, tokens=(new_niece_or_nephew_sim_info, ))
