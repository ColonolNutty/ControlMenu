...
class S4CMSetSimAAsGrandmotherOnMothersSideToSimBOp(S4CMSetSimAAsRelationToSimBOperation):
    """Set Sim A as a grandmother on mothers side of Sim B"""
    ...
    def get_disabled_text(self, grandparent_sim_info: SimInfo, grandchild_sim_info: SimInfo) -> Union[LocalizedString, None]:
        if not CommonSimGenealogyUtils.has_mother(grandchild_sim_info):
            return CommonLocalizationUtils.create_localized_string(S4CMSimControlMenuStringId.SIM_NEEDS_TO_HAVE_A_RELATION_BEFORE_YOU_CAN_ADD_A_RELATION_TO_THEM, tokens=(grandchild_sim_info, S4CMSimControlMenuStringId.MOTHER, self.get_display_name(grandparent_sim_info, grandchild_sim_info)))
    ...
    def run(self, grandparent_sim_info: SimInfo, grandchild_sim_info: SimInfo, on_completed: Callable[[bool], None]=CommonFunctionUtils.noop) -> bool:
        ...
        parent_sim_info = CommonSimGenealogyUtils.get_mother_sim_info(grandchild_sim_info)
