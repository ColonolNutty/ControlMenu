...
class CommonChooseOutfitDialog(CommonChooseDialog):
    ...
    .. code-block:: python
        ...
        def _common_testing_show_choose_outfit_dialog():
            ...
            try:
                ...
                dialog.log.enable()
                outfit_categories = (OutfitCategory.EVERYDAY, 0)
                dialog.show(sim_info, outfit_category_and_index_list=outfit_categories, on_chosen=_on_chosen)
    ...
    def show(
        ...
        on_chosen: Callable[[Union[Tuple[OutfitCategory, int], None], CommonChoiceOutcome], Any]=CommonFunctionUtils.noop,
        ...
        """show(\
            sim_info,
            outfit_category_and_index_list=(),\
        ...
        :type on_chosen: Callable[[Union[Tuple[OutfitCategory, int], None], CommonChoiceOutcome], optional
        ...
        """
        try:
            outfit_list = self._setup_outfit_categories_and_index_list(outfit_list, exclude_outfit_categories)
            return self._show(
                sim_info,
    ...
    def _setup_outfit_categories_and_index_list(
        self,
        outfit_list: Iterator[Tuple[OutfitCategory, int]],
    ...
    def _show(
        ...
        outfit_list: Iterator[Tuple[OutfitCategory, int]]=(),
        thumbnail_type: UiOutfitPicker._OutftiPickerThumbnailType=UiOutfitPicker._OutftiPickerThumbnailType.SIM_INFO,
        on_chosen: Callable[[Union[Tuple[OutfitCategory, int], None], CommonChoiceOutcome], bool]=CommonFunctionUtils.noop
    ):
        def _on_chosen(choice: Tuple[OutfitCategory, int], outcome: CommonChoiceOutcome) -> bool:
            try:
    ...
    def build_dialog(
        ...
        self.log.format_with_message(
            ...
            sim=sim_info,
            outfit_category_and_index_list=outfit_list,
            thumbnail_type=thumbnail_type
        )
        ...
        def _on_chosen(dialog: UiOutfitPicker) -> bool:
            try:
                ...
                self.log.format_with_message('Finished handling outfit items _on_chosen.', result=result)
                return result
            except Exception as ex:
                self.log.error('Error occurred on choosing a value.', exception=ex)
            return False
    ...
    def _setup_dialog_rows(
        ...
        outfit_category_and_index_list: Iterator[Tuple[OutfitCategory, int]]=()
    ):
        self.log.debug('Adding rows.')
        sim_id = CommonSimUtils.get_sim_id(sim_info)
        added_rows = False
        current_outfit = CommonOutfitUtils.get_current_outfit(sim_info)
        for (outfit_category, outfit_index) in outfit_category_and_index_list:
        ...
        try:
            ...
            dialog = UiOutfitPicker.TunableFactory()\
                .default(
                    ...
                    outfit_categories=set([outfit_category_and_index[0] for outfit_category_and_index in outfit_category_and_index_list]),
                    thumbnail_type=thumbnail_type,
                    show_filter=True
                )
