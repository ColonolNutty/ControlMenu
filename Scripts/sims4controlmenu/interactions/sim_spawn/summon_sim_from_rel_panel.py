...
class S4CMSummonSimFromRelPanelInteraction(CommonImmediateSuperInteraction):
    """S4CMSummonSimFromRelPanelInteraction(*_, **__)
    ...
    def get_log_identifier(cls) -> str:
        return 's4cm_spawn_sim_from_rel_panel'
    ...
    def on_started(self, interaction_sim: Sim, interaction_target: Any) -> bool:
        target_location = CommonObjectLocationUtils.get_location(interaction_sim)
        ...
        if CommonSimUtils.get_sim_instance(picked_sim_info) is not None:
            if CommonAgeUtils.is_baby(picked_sim_info):
                ...
                CommonSimLocationUtils.set_location(picked_sim_info, target_location)
        else:
            # Sim is not spawned, so let's spawn them.
            if CommonAgeUtils.is_baby(picked_sim_info):
                from sims.baby.baby_utils import create_and_place_baby
                create_and_place_baby(picked_sim_info, position=target_location.transform.translation, routing_surface=target_location.routing_surface)
            else:
                CommonSimSpawnUtils.spawn_sim(picked_sim_info, location=target_location)
        return True
