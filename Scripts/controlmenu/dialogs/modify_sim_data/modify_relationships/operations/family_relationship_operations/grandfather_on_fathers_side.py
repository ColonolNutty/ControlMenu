...
class CMSetSimAAsGrandfatherOnFathersSideToSimBOp(CMSetSimAAsRelationToSimBOperation):
    ...
    def _display_name(self) -> int:
        return CMSimControlMenuStringId.GRANDFATHER_ON_FATHERS_SIDE
    ...
    def get_disabled_text(self, grandparent_sim_info: SimInfo, grandchild_sim_info: SimInfo) -> Union[LocalizedString, None]:
        if not CommonSimGenealogyUtils.has_father(grandchild_sim_info):
            return CommonLocalizationUtils.create_localized_string(CMSimControlMenuStringId.SIM_NEEDS_TO_HAVE_A_RELATION_BEFORE_YOU_CAN_ADD_A_RELATION_TO_THEM, tokens=(grandchild_sim_info, CMSimControlMenuStringId.FATHER, self.get_display_name(grandparent_sim_info, grandchild_sim_info)))
    ...
    def run(self, grandparent_sim_info: SimInfo, grandchild_sim_info: SimInfo, on_completed: Callable[[bool], None] = CommonFunctionUtils.noop) -> bool:
        ...
        return CMSetSimAAsFatherToSimBOp().run(grandparent_sim_info, parent_sim_info, on_completed=on_completed)
