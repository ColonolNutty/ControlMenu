...
class CMChangeFirstNameOp(CMSingleSimOperation):
    """Change the first name of a Sim."""

    # noinspection PyMissingOrEmptyDocstring
    @property
    def log_identifier(self) -> str:
        return 'cm_change_first_name'
    ...
    def run(self, sim_info: SimInfo, on_completed: Callable[[bool], None] = CommonFunctionUtils.noop) -> bool:
        def _on_chosen(input_value: str, outcome: CommonChoiceOutcome):
            if outcome == CommonChoiceOutcome.ERROR:
                ...
                on_completed(False)
                return
            CommonSimNameUtils.set_first_name(sim_info, input_value)
            on_completed(True)
            return True

        first_name = CommonSimNameUtils.get_first_name(sim_info)
