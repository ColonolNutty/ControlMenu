...
class CMDespawnTargetSimInteraction(CommonImmediateSuperInteraction):
    """CMDespawnTargetSimInteraction(*_, **__)
    ...
    def on_started(self, interaction_sim: Sim, interaction_target: Sim) -> bool:
        target_sim_info = CommonSimUtils.get_sim_info(interaction_target)
        CommonSimSpawnUtils.hard_reset(target_sim_info, reset_reason=ResetReason.RESET_EXPECTED, source=interaction_sim, cause='CM: Preparing To Despawn Sim')
        CommonSimSpawnUtils.despawn_sim(target_sim_info, source=CommonSimNameUtils.get_full_name(CommonSimUtils.get_sim_info(interaction_sim)), cause='CM: Despawn Sim')
        return True
