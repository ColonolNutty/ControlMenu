...
class _CMSetSimAAsMotherToSimBOpTests:
    @staticmethod
    ...
    def _setting_sim_a_as_mother_to_sim_b_should_remove_relationships_of_previous_mother_from_previous_family() -> None:
        ...
        try:
            # Run operation
            CommonAssertionUtils.is_true(CMSetSimAAsMotherToSimBOp()._update_family_tree(sim_a_mother, sim_b_child), 'Failed to update family tree.')
            CMSetSimAAsMotherToSimBOp()._add_relationship_bits(sim_a_mother, sim_b_child)
    ...
    def _has_relation_sim_a_is_mother_of_sim_b() -> None:
        ...
        try:
            ...
            CommonAssertionUtils.is_true(CMSetSimAAsMotherToSimBOp().has_relation(sim_info_a, sim_info_b), 'Failed, Sim A should have been the mother of Sim B but was not.')
    ...
    def _has_relation_sim_a_is_not_mother_of_sim_b() -> None:
        ...
        sim_info_b: SimInfo = CommonSimSpawnUtils.create_human_sim_info()
        try:
            CommonAssertionUtils.is_false(CMSetSimAAsMotherToSimBOp().has_relation(sim_info_a, sim_info_b), 'Failed, Sim A should not have been the mother of Sim B but was.')
        except Exception as ex:
            raise ex
        finally:
            CommonSimSpawnUtils.delete_sim(sim_info_a, cause='CM: testing cleanup')
    ...
    def _has_relation_sim_a_is_only_rel_bit_mother_of_sim_b() -> None:
        ...
        try:
            ...
            CommonAssertionUtils.is_false(CMSetSimAAsMotherToSimBOp().has_relation(sim_info_a, sim_info_b), 'Failed, Sim A should not have been the mother of Sim B with only the relationship bit but was.')
        except Exception as ex:
            raise ex
        finally:
            CommonSimSpawnUtils.delete_sim(sim_info_a, cause='CM: testing cleanup')
    ...
    def _has_relation_sim_a_is_only_family_tree_mother_of_sim_b() -> None:
        ...
        try:
            ...
            CommonAssertionUtils.is_false(CMSetSimAAsMotherToSimBOp().has_relation(sim_info_a, sim_info_b), 'Failed, Sim A should not have been the mother of Sim B with only the family tree but was.')
        except Exception as ex:
            raise ex
        finally:
            CommonSimSpawnUtils.delete_sim(sim_info_a, cause='CM: testing cleanup')
